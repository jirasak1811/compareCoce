    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                if (result) {
                    var sql = 'SELECT '
                        + 'extensions.extension , extensions.name , organizations.org_name ,extensions.rent_dial_number_fee ,extensions.special_service_fee ,extensions.email , extensions.authorization_code '
    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                if (result) {
                    var sql = 'SELECT '
                        + 'extensions.extension, extensions.name , organizations.org_name ,extensions.email '
                        + 'FROM extensions LEFT JOIN organizations ON organizations.orgid=extensions.orgid WHERE extensions.clientid = ? AND extensions.extension LIKE ? OR extensions.nameth LIKE ? OR extensions.name LIKE ? OR extensions.email LIKE ? OR organizations.org_path LIKE ? ORDER BY extensions.extension '
                    var values = [
                        result[0].clientid,
                        '%' + req.params.keyword + '%',
                        '%' + req.params.keyword + '%',
    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                if (result) {
                    var sql = 'SELECT '
                        + 'extensions.extension , extensions.exttype , extensions.name, extensions.authorization_code, extensions.email , organizations.org_name , extensions.rent_charge, extensions.special_service_fee, extensions.has_license ,extensions.id '
                        + 'FROM extensions LEFT JOIN organizations ON organizations.orgid=extensions.orgid WHERE extensions.clientid = ? ORDER BY extensions.extension asc'
                    var values = [
                        result[0].clientid
                    ];
    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                //console.log(result[0].clientid);
                if (result) {
                    var sql = "select extensions.extension, extensions.name, extensions.authorization_code, organizations.org_name, extensions.email From extensions "
                    + "LEFT JOIN organizations ON organizations.orgid = extensions.orgid "
                    var values = [
                        result[0].clientid
                    ];
...
router.get('/search/user/detail/:keyword', function (req, res, next) {
    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                if (result) {
                    var sql = 'SELECT '
                        + 'extensions.extension , extensions.name ,extensions.rent_dial_number_fee ,extensions.special_service_fee ,extensions.email , extensions.authorization_code '
                        + 'FROM extensions LEFT JOIN organizations ON organizations.orgid=extensions.orgid WHERE extensions.clientid = ? AND extensions.extension = ? '
                    var values = [
                        result[0].clientid,
                        req.params.keyword
...
router.get('/search/guest/:keyword', function (req, res, next) {
    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            var sql = 'SELECT '
                + 'extensions.extension , extensions.nameth , extensions.name ,extensions.email '
