...
router.post('/usergroup', function (req, res, next) {
    database.getConnection(function (err, conn) {
        if (conn) {
            conn.query({
                sql: 'SELECT groupview FROM users WHERE username = ? ',
                values:[ req.body.username ]
            }, function (err, rows) {
                conn.release();
                if (rows) {
                    res.send(JSON.stringify(rows[0].groupview));
                } else {
                    return next(err);
                }
            });
...
router.post('/usergroup/setgroup', function (req, res, next) {
    database.getConnection(function (err, conn) {
        if (conn) {
            conn.query({
                sql: 'UPDATE users SET groupview = ? WHERE username = ?',
                values: [req.body.groupview, req.body.username]
            }, function (err, rows) {
                conn.release();
                if (rows) {
                    res.send(JSON.stringify(rows));
