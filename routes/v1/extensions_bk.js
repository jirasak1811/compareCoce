...
router.post('/update', function (req, res, next) {
    ...
    var emp_id = req.body.pk.split(',')[1]
    var name = req.body.pk.split(',')[2]
    req.body.pk = req.body.pk.split(',')[0]
    var sql = ''
    var values
    if ((name == 'null' || name == '-') && (emp_id == 'null' || emp_id == '-')) {
        sql = 'UPDATE extensions SET ??=? WHERE extension = ?  ';
        values = [
            req.body.name,
            req.body.value.trim() ? req.body.value.trim() : null,
            req.body.pk
        ];
    } else if ((name != 'null' || name != '-' )&& (emp_id == 'null' || emp_id == '-')) {
        sql = 'UPDATE extensions SET ??=? WHERE extension = ? AND name = ? ';
        values = [
            req.body.name,
            req.body.value.trim() ? req.body.value.trim() : null,
            req.body.pk,
        ...
        sql = 'UPDATE extensions SET ??=? WHERE extension = ? AND emp_id = ? ';
        values = [
            req.body.name,
            req.body.value.trim() ? req.body.value.trim() : null,
            req.body.pk,
        ...
        sql = 'UPDATE extensions SET ??=? WHERE extension = ? AND emp_id = ? AND name = ? ';
        values = [
            req.body.name,
            req.body.value.trim() ? req.body.value.trim() : null,
            req.body.pk,
