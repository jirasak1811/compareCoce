    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                if (result) {
                    var sql = 'SELECT '
                        + 'extensions.emp_id,extensions.extension , extensions.exttype , extensions.nameth , extensions.name ,extensions.employee_line, extensions.position , extensions.email , organizations.org_name , extensions.rent_charge , extensions.special_service_fee ,extensions.id '
                        ...
                        + 'FROM extensions LEFT JOIN organizations ON organizations.orgid=extensions.orgid WHERE extensions.clientid = ? ORDER BY extensions.extension '
                    var values = [
                        result[0].clientid
                    ];
    ...
    database.getConnection(function (err, conn) {
        if (conn) {
            ...
            conn.query({
                ...
                //console.log(result[0].clientid);
                if (result) {
                    var sql = "select extensions.extension, extensions.name, extensions.nameth, organizations.org_name, extensions.position, extensions.email, extensions.employee_type From extensions "
