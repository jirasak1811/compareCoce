...
function getAllGroup2(user, ext_lic, token) {
    ...
    $.getJSON('/api/v1/organize/all', { user: user }).done(function (data) {
        ...
        //$.each(data, function (i, d) {
        //    //console.log(d);
        //    //var org = '<a dclass="btn btn-app" href="/api/v1/organize/all/'+d.orgid+'">';
        //    org = '';
...
function RenderTable(data) {
    if (data.length != 0) {
        ...
        tmp += "    </tr> </thead>"
        $.each(data, function (i, d) {
            tmp += "    <tr class='ext-list-item showmoredetail' data-pk=" + d.extension+">"
...
$(document).ready(function () {
    ...
    $(document).on('click', '.ext-list-item', function () {
        ...
        if (!cntrlIsPressed) {
            for (var i = 0; i < document.getElementsByClassName('ext-list-item').length; i++) {
                document.getElementsByClassName('ext-list-item')[i].className = "ext-list-item showmoredetail"
            }
            tmpExtSelect = []
        }
