...
$(document).ready(function () {
    ...
    $(document).on('dblclick', '.showmoredetail', function () {
        ...
        var extension = $(this).attr('data-pk')
        $.getJSON('/api/v1/extensions/info/' + $(this).attr('data-pk'), { user: Cookies('user') }).done(function (data) {
            console.log(data[0])
            if (data.length == 1) {
...
function getAllExtension(user, ext_lic, token) {
    $.getJSON('/api/v1/directorysearch/all/extension', { user: user }).done(function (data) {
        var ext = '<div class="row"> <div class="col-md-6"><a class="btn btn-app btn-ext-add bg-olive btn-flat" >';
        ...
        ext += '</a></div><div class="col-md-6"><input class="form-control" type="text" id="searchextenstion" onkeyup="searchtable('+"'"+user+"'"+')" placeholder="Search...." data-i18n="[placeholder]search"></div></div>';

        $('.ext-list').append(ext);
        RenderExtensionTable(data)
        if (document.getElementsByTagName('html')[0].lang == 'th') {
            $('.with-border').append('<button type="button" class="btn-ext-add-range btn bg-purple"> <i class="fa fa-plus-square"></i><label data-i18n="Create Extension Range">สร้างเบอร์ภายในแบบกำหนดช่วง</label></button>')
        } else {
            $('.with-border').append('<button type="button" class="btn-ext-add-range btn bg-purple"> <i class="fa fa-plus-square"></i><label data-i18n="Create Extension Range">Create Extension Range</label></button>')
        }
        if (document.getElementsByTagName('html')[0].lang == 'th') {
            $('.lang-switch')[1].click()
        } else {
...
function RenderExtensionTable(data) {
    ...
    $.each(data, function (i, d) {
        tmp =''
        tmp += "    <tr editindex='-' class='showmoredetail INL" + d.extension + "' data-pk='" + d.extension + "'>"
        $.each(Object.keys(data[0]), function (i, keyname) {
            ...
            if (keyname.toString() == 'org_path' || keyname.toString() == 'org_name') {
                tmp += "<td> <a class='INL" + d.extension + "'" + 'data-type="text" editindex="' + (editindex++) + '" data-pk="' + d.extension + '" data-name="' + keyname.toString() + '" data-title="Enter ' + keyname.toString() + '" >' + (d[keyname.toString()] || '-') + "</a></td>"
            } else if (keyname.toString() == 'has_license') {
                tmp += "<td> <a class='editable-selectActive INL" + d.extension + "'" + 'data-value="' + d[keyname.toString()] + '" data-type="select" editindex="' + (editindex++) + '"  data-pk="' + d.extension + '" data-name="' + keyname.toString() + '" data-title="Enter ' + keyname.toString() + '" >' + (d[keyname.toString()] == '1' ? 'Active' : 'InActive') + "</a></td>"
            } else if (keyname.toString() == 'exttype') {
                tmp += "<td> <a class='editable-selectexttype INL" + d.extension + "'" + 'data-type="select" editindex="' + (editindex++) + '"  data-pk="' + d.extension + '" data-name="' + keyname.toString() + '" data-title="Enter ' + keyname.toString() + '" >' + (d[keyname.toString()] || '-') + "</a></td>"
            } else if (keyname.toString() == 'rent_charge' || keyname.toString() == 'special_service_fee') {
                tmp += "<td> <a class='editable INL" + d.extension + "'" + 'data-type="text" editindex="' + (editindex++) + '" data-pk="' + d.extension + '" data-name="' + keyname.toString() + '" data-title="Enter ' + keyname.toString() + '" >' + d[keyname.toString()] + "</a></td>"
            }
            else {
                tmp += "<td> <a class='editable INL" + d.extension + "'" + ' style="word-break:break-all;" data-type="text" editindex="' + (editindex++) + '" data-pk="' + d.extension + '" data-name="' + keyname.toString() + '" data-title="Enter ' + keyname.toString() + '" >' + (d[keyname.toString()] || '-') + "</a></td>"
            }
        });
        //Action 
        tmp += "<td align='center' style='text-align:center;width:30px'> <a class='btn btn-warning btn-xs btn-delete-extension INL" + d.extension + "' editindex='-'  data-pk='" + d.extension + "' '><i class='fa fa-trash'></i></a></td>"
        tmp += "</tr>"
...
function updateEditable() { 
    var editabledata = {
        ...
        ajaxOptions: {
            ...
            if ($(this).attr('data-name') == 'extension') {
