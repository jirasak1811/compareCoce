...
function getRecentCalls() {
    $.getJSON('/api/v1/dashboard/recent/calls').done(function (data) {
        mapRecentCallsData(data)
    });
}

function mapRecentCallsData(data) {
    ...
    $.each(data, function (i, d) {
        ...
        if (maxrows && ((showOut && d.callType == 'OUT') || (showIn && d.callType == 'IN'))) {
            ...
            row += '<td class=\'hidden-xs text-right\'>' + (d.route ? d.route : '-') + "/" + (d.trunk ? d.trunk : '-') + '</td>'; // Add Trunk Data
            row += '</tr>';
...
function mapChartData(data) {
    ...
    for (var i = 0; i < ChartData.length; i++) {
        if (ChartData[Object.keys(ChartData)[i]] != undefined) {
            mapChart.push({ y: ChartData[Object.keys(ChartData)[i]].Time, a: ChartData[Object.keys(ChartData)[i]].In, b: ChartData[Object.keys(ChartData)[i]].Out })
        }
    }
    $('#bar-chart').html('')
    RenderBar(mapChart)
...
$(document).ready(function () {
    if (io != undefined) {
        const socket = io();
        socket.on('dashboard-systeminfo', (data) => {
            data = JSON.parse(data)
            console.log("dashboard-systeminfo : ", data)
            mapSystemInfo(data)
        })
        socket.on('dashboard-chart', (data) => {
            data = JSON.parse(data)
            console.log("dashboard-chart : ", data)
            mapChartData(data)
        })
        socket.on('dashboard-recentscall', (data) => {
            data = JSON.parse(data)
            console.log("dashboard-recentscall : ", data)
            mapRecentCallsData(data)
        })
    }
})